...
abstract class BaseListPostFragment<ITEMBEN,ITEMVIEW:View>: BaseFragment(),BaseView,ResponseError {
    val gson by lazy { Gson() }
    val persenter by lazy { BasePersenterImpl(this,context,this) }
    val imageAdapter by lazy { getNowAdapter() }
    ...
    override fun initListener() {
        ...
        referLayout.setOnRefreshListener {
            persenter.loadDataPost(getRequestUrl(0),getRequestParam(), UtilEnum.REFER)
        }
            ...
            override fun onScrollStateChanged(recyclerView: RecyclerView?, newState: Int) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE){
                    ...
                    if(layoutManager is LinearLayoutManager){
                        ...
                        if(lastPosition == imageAdapter.itemCount-1){
                            persenter.loadDataPost(getRequestUrl(lastPosition),getRequestParam(), UtilEnum.LOAD)
                        }
                    }
                }
            }
    ...
    override fun loadMoreSuccess(result:String) {
        imageAdapter.LoadMore(getListData(result))
    }
    override fun loadSuccess(result:String) {
        imageAdapter.updateList(getListData(result))
        closeRefer()
    }
    ...
    abstract fun getRequestParam():String
}